...
class DDIMSampler_trainer(object):
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        # 分离状态字典中的Tensor，使其成为独立的副本
        # for key in list(optimizer_state.keys()):
            # if isinstance(optimizer_state[key], torch.Tensor):
                # optimizer_state[key] = optimizer_state[key].detach()
...
class OurDDIMSampler_trainer(object):
    def __init__(self, model, quant_model, lr_scheduler, optimizer, schedule="linear", **kwargs):
        super().__init__()
        self.model = model
        self.quant_model = quant_model
        ...
        self.fp_outputs = []
        self.quant_outputs = []
        def hook_fn_quant(module, input, output):
            # 保存输出到列表中
            self.quant_outputs.append(output.clone().detach())

        def hook_fn_fp(module, input, output):
            # 保存输出到列表中
            self.fp_outputs.append(output.clone().detach())

        self.model.model.diffusion_model.output_blocks[-1].register_forward_hook(hook_fn_fp)
        self.quant_model.model.diffusion_model.model.output_blocks[-1].register_forward_hook(hook_fn_quant)
        self.distill_step = 0
