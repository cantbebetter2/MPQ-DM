...
from quant_scripts.quant_layer import QuantModule, StraightThrough, QuantModule_intnlora, QuantModule_lora
...
class QuantModel_lora(nn.Module):
        ...
        for name, child_module in module.named_children():
            if isinstance(child_module, (nn.Conv2d, nn.Linear)) and 'skip' not in name and 'op' not in name:
                ...
                if self.count in self.special_module_count_list:
                    ...
                    setattr(module, name, QuantModule_lora(child_module, weight_quant_params, act_quant_params, num_steps=self.num_steps))
    ...
    def set_quant_state(self, weight_quant: bool = False, act_quant: bool = False):
        for m in self.model.modules():
            if isinstance(m, (QuantModule_lora, QuantModule)):  ## remove BaseQuantBlock
                m.set_quant_state(weight_quant, act_quant)
    ...
    def set_first_last_layer_to_8bit(self):
        module_list = []
        for m in self.model.modules():
            if isinstance(m, (QuantModule_lora, QuantModule)):
                module_list += [m]
        module_list[0].weight_quantizer.bitwidth_refactor(8)
        module_list[0].act_quantizer.bitwidth_refactor(8)
